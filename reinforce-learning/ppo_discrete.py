...
class PPO_discrete:
    ...
    def evaluate(self, s,invailid_choice):  # When evaluating the policy, we select the action with the highest probability
        ...
        a_prob[invailid_choice] = torch.tensor(-1e8)
        # a_prob = a_prob
        a = np.argmax(a_prob)
        return a


    def choose_action(self, s,invalid_action=None):
        s = torch.unsqueeze(torch.tensor(s, dtype=torch.float), 0)
        with torch.no_grad():
            probs = self.actor(s)
            if invalid_action is not None:
                temp_mask = np.ones(parking_lot_num+1,dtype=int)
                temp_mask[invalid_action] = 0
                invalid_action_mask = torch.tensor(temp_mask)
                invalid_action_mask = invalid_action_mask.type(torch.BoolTensor)
                adjust_logits = torch.where(invalid_action_mask, probs, torch.tensor(-1e8))
                dist = Categorical(logits=adjust_logits)
            a = dist.sample()
            a_logprob = dist.log_prob(a)
        return a.numpy()[0], a_logprob.numpy()[0]

    def update(self, replay_buffer, total_steps):
        s,a,a_logprob,r,s_,done = replay_buffer.numpy_to_tensor()  # Get training data
        """
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                entropy = entropy + sum(dist_entropy)/len(index)
        ...
        return cri_loss/self.K_epochs,entropy/self.K_epochs
